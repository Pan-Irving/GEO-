...
import type { ContentPlan, CustomSourceBatchPayload, CustomSourcePayload, IntentGroup, MarkdownArticleImportMeta, MatrixImportDraft, Project, PublishingUsageSummary, WorkflowStep } from "./api/types";
    ...
    <div className="modal-backdrop">
      <div className="content-plan-modal intent-group-modal" role="dialog" aria-modal="true" aria-labelledby="intent-group-title">
        <header className="modal-head">
          <div>
            <p className="eyebrow">项目标准库</p>
            <h2 id="intent-group-title">意图簇管理</h2>
            <span>基于归档文章或当前内容矩阵维护标准意图簇；后续矩阵生成会优先使用这里的名称和关键词归属。</span>
          </div>
          <button className="icon-btn" onClick={onClose}><X size={18} /></button>
        </header>

        <div className="intent-group-toolbar">
          <button className="btn primary" onClick={() => setEditingId("__new__")}><Plus size={15} />新建意图簇</button>
          <button className="btn" onClick={onRebuild}><RefreshCw size={15} />从当前数据重建</button>
          <span>{groups.length} 个意图簇</span>
        </div>
        ...
        {!groups.length && !creating ? (
          ...
          <div className="intent-group-layout">
            <aside className="intent-group-list">
              ...
              {groups.map(group => (
                <button key={group.id} className={group.id === editingId ? "active" : ""} onClick={() => setEditingId(group.id)}>
                  ...
                  <span>{group.keywords.length} 个关键词 · {group.article_count} 篇归档</span>
                </button>
              ))}
            </aside>
            ...
            <section className="intent-group-editor">
              ...
              <label>
                <span>历史别名（一行一个）</span>
                <textarea value={aliasesText} onChange={event => setAliasesText(event.target.value)} rows={4} />
              </label>
              ...
              {!creating && (
              <div className="intent-group-merge">
                <h3>合并重复簇</h3>
                <select value={mergeTargetId} onChange={event => setMergeTargetId(event.target.value)}>
                  {groups.map(group => <option key={group.id} value={group.id}>合并到：{group.name}</option>)}
                </select>
