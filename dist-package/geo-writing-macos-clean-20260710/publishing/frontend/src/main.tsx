...
const MATRIX_ARTICLE_TYPES = ["榜单推荐文", "横评对比文", "支柱标准文", "场景选购文"];
const INDEPENDENT_ASSET_TYPES = ["产品证据文", "FAQ问答文"];
const ARTICLE_TYPES = [...MATRIX_ARTICLE_TYPES, ...INDEPENDENT_ASSET_TYPES];
          ...
          <>
            ...
            {view === "records" && inventory && (
              <Records
                ...
                projectName={activeProject?.project_name || "发布记录"}
                isAdmin={isAdmin}
                currentUser={user}
...
function Dashboard({
  ...
  const independentAssets = inventory.articles.filter(article => isIndependentAssetType(article.article_type));
  return (
    <>
      ...
      <section className="panel matrix-panel">
        <div className="panel-head">
          <div><h2>意图簇 × 文章类型库存</h2><p>{isAdmin ? "按定稿、已使用和采购中统计；当定稿减已使用不足 2 篇时提示补产。" : "按本人可见文章统计定稿、已使用和采购中；当定稿减已使用不足 2 篇时提示补产。"}</p></div>
          <span className="badge warn">{lowCells.length} 个低库存位</span>
        </div>
        <MatrixTable cells={inventory.matrix} onOpenArticles={onOpenArticles} />
      </section>
      <IndependentAssets articles={independentAssets} onOpenArticles={onOpenArticles} />
      <section className="panel signal-panel">
        <div className="panel-head"><div><h2>补产信号</h2><p>定稿减已使用剩余 0 或 1 篇的意图簇类型建议回到撰文系统补产。</p></div></div>
        <div className="signal-list">
          {lowCells.length ? lowCells.map(cell => (
            <div className="signal" key={`${cellIntentGroup(cell)}-${cell.article_type}`}>
              <strong>{cellIntentGroup(cell)}</strong>
...
function MatrixTable({
  ...
  const intentGroups = unique(cells.map(cellIntentGroup));
  const types = MATRIX_ARTICLE_TYPES;
  const map = new Map(cells.map(cell => [`${cellIntentGroup(cell)}\u0001${normalizeArticleType(cell.article_type)}`, { ...cell, article_type: normalizeArticleType(cell.article_type) }]));
  return (
    <div className="table-wrap">
      <table className="matrix">
        <thead><tr><th>意图簇</th>{types.map(type => <th key={type}>{type}</th>)}</tr></thead>
        <tbody>
          {intentGroups.map(intentGroup => (
            <tr key={intentGroup}>
              <td className="keyword-cell">{intentGroup}</td>
              {types.map(type => {
                const cell = map.get(`${intentGroup}\u0001${type}`);
                return (
                  <td key={type}>
                    {cell ? (
                      <button
                        ...
                        type="button"
                        title={`查看文章库：${intentGroup} / ${type}`}
                        onClick={() => onOpenArticles({ intentGroup, articleType: type })}
                      >
...
function IndependentAssets({
  ...
  const grouped = groupBy(articles, article => normalizeArticleType(article.article_type));
  const total = articles.length;
  return (
    <section className="panel independent-assets-panel">
      <div className="panel-head">
        <div>
          <h2>独立内容资产</h2>
          <p>产品证据文按卖点举证，FAQ 按真实问题沉淀，不纳入上方意图簇矩阵和低库存信号。</p>
        </div>
        <span className="badge accent">{total} 项资产</span>
      </div>
      <div className="independent-assets-grid">
        {INDEPENDENT_ASSET_TYPES.map(type => {
          const items = grouped[type] || [];
          const available = items.filter(article => (article.inventory_status || "可使用") === "可使用").length;
          const purchasing = items.filter(article => (article.inventory_status || "可使用") === "采购中").length;
          const used = items.filter(article => (article.inventory_status || "可使用") === "已使用").length;
          return (
            <article className="independent-asset-group" key={type}>
              <header>
                <div>
                  <strong>{type}</strong>
                  <span>{type === "FAQ问答文" ? "真实问题 / 问答资产" : "卖点 / 证据资产"}</span>
                </div>
              </header>
              <div className="asset-stats">
                <span className="badge">{items.length} 定稿</span>
                <span className="badge good">{available} 可使用</span>
                <span className="badge accent">{used} 已使用</span>
                <span className="badge warn">{purchasing} 采购中</span>
              </div>
              <div className="asset-list">
                {items.map(article => (
                  <button
                    className={`asset-item status-${assetStatusClass(article)}`}
                    type="button"
                    key={article.article_id}
                    title={`查看文章库：${type}`}
                    onClick={() => onOpenArticles({ articleType: type })}
                  >
                    <strong>{article.title}</strong>
                    <span>{article.inventory_status || "可使用"} · {articleIntentGroup(article)}</span>
                  </button>
                ))}
                {!items.length && <span className="empty-inline">暂无{type}</span>}
              </div>
            </article>
          );
        })}
...
function articleIntentGroup(article: ArticleSnapshot): string {
  return article.intent_group || article.keyword || "未归类意图簇";
}

function recordIntentGroup(record: PublicationRecord): string {
  return record.intent_group || record.keyword || "未归类意图簇";
}

function cellIntentGroup(cell: MatrixCell): string {
  return cell.intent_group || cell.keyword || "未归类意图簇";
}
...
function isIndependentAssetType(value: string): boolean {
  return INDEPENDENT_ASSET_TYPES.includes(normalizeArticleType(value));
}

function assetStatusClass(article: ArticleSnapshot): "available" | "purchasing" | "used" {
  const status = article.inventory_status || "可使用";
  if (status === "采购中") return "purchasing";
...
function ArticleLibrary({
  ...
  useEffect(() => {
    if (!focus) return;
    setQuery("");
    setKeywordFilter(focus.intentGroup || "");
    setTypeFilter(focus.articleType);
    setStatusFilter("");
    setFinalizedDateFilter("");
    setSelectedArticleIds(new Set());
  }, [focus]);
  const keywordOptions = unique(inventory.articles.map(articleIntentGroup));
  ...
  const filtered = inventory.articles.filter(article => {
    ...
    return matchesQuery
      ...
      && (!finalizedDateFilter || finalizedDate === finalizedDateFilter);
  });
  const filteredIds = useMemo(() => new Set(filtered.map(article => article.article_id)), [filtered]);
  const selectedArticles = filtered.filter(article => selectedArticleIds.has(article.article_id));
  useEffect(() => {
    setSelectedArticleIds(current => {
      const next = new Set([...current].filter(articleId => filteredIds.has(articleId)));
  ...
  const grouped = groupBy(filtered, articleIntentGroup);
  const hasFilters = Boolean(query || keywordFilter || typeFilter || statusFilter || finalizedDateFilter);
      ...
      <div className="panel library-filter-card">
        <div className="panel-body toolbar library-filters">
          <label className="search">
            <Search size={15} />
            <input value={query} placeholder="搜索文章标题、意图簇、文章类型或媒体" onChange={event => setQuery(event.target.value)} />
          </label>
          <select className="filter-select" value={keywordFilter} onChange={event => setKeywordFilter(event.target.value)}>
            <option value="">全部意图簇</option>
          ...
          <label className="date-filter-field" aria-label="定稿时间">
            <input type="date" value={finalizedDateFilter} onChange={event => setFinalizedDateFilter(event.target.value)} />
          </label>
          <button className="btn reset-filter-btn" disabled={!hasFilters} onClick={resetFilters}><RefreshCw size={15} />重置</button>
          <button className="btn primary bulk-download-btn" disabled={!selectedArticles.length} onClick={() => downloadMarkdownBatch(selectedArticles)}><Download size={15} />批量下载</button>
          <span className="filter-summary">{filtered.length} / {inventory.articles.length} 篇 · 已选 {selectedArticles.length} 篇</span>
        </div>
      </div>

      <div className="library">
        {Object.entries(grouped).map(([intentGroup, articles], index) => (
          <section className="library-group" key={intentGroup}>
            ...
            {articles.map(article => (
              <article className={`article-card ${usedArticleIds.has(article.article_id) ? "is-used" : ""}`} key={article.article_id}>
                <label className="article-select" title="选择文章">
                  <input type="checkbox" checked={selectedArticleIds.has(article.article_id)} onChange={() => toggleArticleSelection(article.article_id)} />
                </label>
                <div className="article-main">
                  <div className="article-meta">
                    ...
                    <span className="finalized-time-chip">定稿时间 {articleFinalizedTimeLabel(article) || "-"}</span>
...
function Records({
  ...
  const filtered = records.filter(record => {
    ...
    return (!normalizedQuery || searchable.includes(normalizedQuery))
      ...
      && (!dateStartFilter || recordDate >= dateStartFilter)
      && (!dateEndFilter || recordDate <= dateEndFilter)
      && (!employeeFilter || employee === employeeFilter);
  });
  const publishedCount = filtered.filter(record => record.order_status === "published").length;
  const purchasingCount = filtered.filter(record => record.order_status === "purchasing").length;
  const selfPublishedCount = filtered.filter(record => record.order_status === "published" && isSelfChannel(record.channel_type)).length;
  const publishedWebCost = filtered
  ...
  const grouped = groupBy(filtered, record => `${recordIntentGroup(record)}\u0001${normalizeArticleType(record.article_type || "未标注类型")}`);
  const hasFilters = Boolean(query || keywordFilter || typeFilter || channelFilter || statusFilter || dateStartFilter || dateEndFilter || employeeFilter);
  const exportRows = filtered.map(record => ({
    日期: recordDateLabel(record),
    文章: record.title || record.article_id,
    渠道: record.channel_type || "-",
    媒体: record.media_name || "-",
    发布人: record.employee_name || "未标注员工",
    关联AI: (record.target_ai_platforms || []).join("、"),
    状态: statusLabel(record.order_status),
    实际成本: record.actual_cost ? record.actual_cost : "",
    发布链接: record.publish_url || "",
    操作: recordActionLabel(record, isAdmin, currentUser),
  }));
  return (
    <div className="records-page">
      ...
      <section className={`record-filter-card ${isAdmin ? "admin-record-filter" : "user-record-filter"}`}>
        <div className="toolbar">
          <label className="search"><Search size={15} /><input value={query} placeholder={isAdmin ? "搜索文章、意图簇、媒体、员工或链接" : "搜索我的文章、意图簇、媒体或链接"} onChange={event => setQuery(event.target.value)} /></label>
          <select className="filter-select" value={keywordFilter} onChange={event => setKeywordFilter(event.target.value)}>
            <option value="">全部意图簇</option>
            {keywordOptions.map(intentGroup => <option key={intentGroup} value={intentGroup}>{intentGroup}</option>)}
          </select>
          ...
          <button className="text-btn export-btn" type="button" onClick={() => void exportPublicationRecords(exportRows, projectName)}>导出</button>
      ...
      <section className="record-filter-card">
        <div className="work-log-filters">
          <div className="work-log-filter-row primary">
            <label className="search"><Search size={15} /><input value={query} placeholder={isAdmin ? "搜索文章、意图簇、媒体或员工" : "搜索我的文章、意图簇或媒体"} onChange={event => setQuery(event.target.value)} /></label>
            {isAdmin && (
      ...
      <section className="record-group">
        ...
        <div className="table-wrap work-log-table-wrap">
          {filtered.length ? <table className="record-table work-log-table">
            ...
            <thead><tr><th>日期</th><th>员工</th><th>文章</th><th>意图簇</th><th>文章类型</th><th>渠道</th><th>媒体</th><th>状态</th></tr></thead>
            <tbody>
              {filtered.map(record => (
                <tr key={record.id}>
                  <td>{recordDateLabel(record)}</td>
                  <td>{record.employee_name || "未标注员工"}</td>
                  <td><strong>{record.title || record.article_id}</strong></td>
                  <td>{recordIntentGroup(record)}</td>
...
function AdminPanel({
  ...
  const [assignment, setAssignment] = useState<AssignmentDraft>({ user_id: "", project_id: projects[0]?.project_id || "", intent_group_ids: [], intent_groups: [], article_types: [] });
  const [assignmentOptions, setAssignmentOptions] = useState<AssignmentOptions>({ intentGroups: [], articleTypes: [] });
  const [assignmentIntentGroupIdsByName, setAssignmentIntentGroupIdsByName] = useState<Record<string, string>>({});
  ...
  useEffect(() => {
    if (!assignment.project_id) {
      setAssignmentOptions({ intentGroups: [], articleTypes: [] });
      setAssignmentIntentGroupIdsByName({});
    ...
    void api.inventory(assignment.project_id)
      .then(response => {
        ...
        response.articles.forEach(article => {
          const name = articleIntentGroup(article);
          if (name && article.intent_group_id) idByName[name] = article.intent_group_id;
        });
        setAssignmentIntentGroupIdsByName(idByName);
        setAssignmentOptions({
          intentGroups: unique(response.articles.map(articleIntentGroup)),
  ...
  function resolveAssignmentIntentGroupIds(intentGroups: string[], projectId: string): string[] {
    if (projectId !== assignment.project_id) return [];
    return unique(intentGroups.map(group => assignmentIntentGroupIdsByName[group]).filter(Boolean));
  }
  ...
  return (
    <div className="admin-grid">
      ...
      <section className="panel wide">
        <div className="panel-head"><div><h2>项目板块分配</h2><p>意图簇和文章类型留空表示该项目内全部可见。</p></div></div>
        <div className="assignment-grid">
          ...
          <MultiValueInput
            value={assignment.intent_groups}
            options={assignmentOptions.intentGroups}
            placeholder={assignmentOptionsLoading ? "正在加载意图簇候选..." : "选择或输入意图簇"}
            emptyText={assignment.project_id ? "暂无意图簇候选，可手动输入" : "请先选择项目"}
            onChange={intentGroups => setAssignment({ ...assignment, intent_groups: intentGroups })}
          />
          <MultiValueInput
          ...
          <button className="btn primary admin-action-btn" onClick={() => void run(async () => {
            const intentGroupIds = resolveAssignmentIntentGroupIds(assignment.intent_groups, assignment.project_id);
            await api.createAssignment({
              user_id: assignment.user_id,
              project_id: assignment.project_id,
              intent_group_ids: intentGroupIds,
              intent_groups: assignment.intent_groups,
              article_types: assignment.article_types
            });
            setAssignment({ ...assignment, intent_group_ids: [], intent_groups: [], article_types: [] });
            await reload();
          }, "负责板块已分配。")}>保存分配</button>
        </div>
        ...
        <div className="table-wrap">
          <table>
            <thead><tr><th>员工</th><th>项目</th><th>意图簇</th><th>文章类型</th><th>操作</th></tr></thead>
            <tbody>{assignments.map(item => (
              <tr key={item.id}>
                ...
                <td>{(item.intent_groups?.length ? item.intent_groups : item.keywords).length ? (item.intent_groups?.length ? item.intent_groups : item.keywords).join("、") : "全部"}</td>
...
function recordActionLabel(record: PublicationRecord, isAdmin: boolean, currentUser: User): string {
  const canDelete = isAdmin || record.employee_id === currentUser.id;
  const selfChannel = isSelfChannel(record.channel_type);
  const actions: string[] = [];
  if (selfChannel && canDelete) actions.push("修改");
  if (isAdmin && !selfChannel && record.order_status === "published") actions.push("修改");
  if (isAdmin && !selfChannel && record.order_status === "purchasing") actions.push("回填");
  if (canDelete) actions.push("撤销");
...
function downloadMarkdown(article: ArticleSnapshot) {
  ...
  link.href = url;
  link.download = `${safeDownloadName(article.title || article.article_id)}.md`;
  link.click();
  URL.revokeObjectURL(url);
}

function downloadMarkdownBatch(articles: ArticleSnapshot[]) {
  articles.forEach((article, index) => {
    window.setTimeout(() => downloadMarkdown(article), index * 120);
  });
}

function articleFinalizedDateValue(article: ArticleSnapshot): string {
  return dateOnlyValue(article.article_audited_at || "");
}

function articleFinalizedTimeLabel(article: ArticleSnapshot): string {
  return shortDateTimeLabel(article.article_audited_at || "");
}

function shortDateTimeLabel(value: string): string {
  if (!value) return "";
  ...
  return value.replace("T", " ").slice(5, 16).replace("-", "/");
}

function dateOnlyValue(value: string): string {
  if (!value) return "";
...
function safeDownloadName(value: string): string {
  return value.trim().replace(/[\\/:*?"<>|]/g, "_") || "article";
}

async function exportPublicationRecords(rows: Array<Record<string, string | number>>, projectName: string) {
  const headers = ["日期", "文章", "渠道", "媒体", "发布人", "关联AI", "状态", "实际成本", "发布链接", "操作"];
  const sheetData: SheetData = [
    headers.map(header => ({ value: header, type: String, fontWeight: "bold" })),
    ...rows.map(row => headers.map(header => row[header] ?? ""))
  ];
  const date = todayDateInputValue().replace(/-/g, "");
  const safeProjectName = projectName.trim().replace(/[\\/:*?"<>|]/g, "_") || "发布记录";
  ...
  }).toFile(`${safeProjectName}-发布记录-${date}.xlsx`);
}
