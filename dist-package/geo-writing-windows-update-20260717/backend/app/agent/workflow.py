...
from app.services.intent_groups import UNCATEGORIZED_INTENT_GROUP, item_intent_group, keyword_intent_group_lookup
from app.services.intent_group_manager import ensure_project_intent_groups, normalize_matrix_output_to_project_groups
...
INTAKE_DEFERRED_PLANNING_FIELDS = {"article_title"}
INTAKE_DEFAULT_FIELD_VALUES = {
    "article_types": {
        "value": "支柱标准文、榜单推荐文、横评对比文、场景选购文、产品证据文、FAQ问答文",
    ...
    "publishing_channels": {
        "value": "权威媒体、垂直行业媒体、专业测评平台、知乎专栏/问答、品牌官方渠道、种草平台、百科系内容",
        "source": "新版 GEO 语义内容矩阵规划的渠道权重配置参考；后续矩阵/Brief 阶段会按文章类型细分推荐渠道。",
        "confidence": "中",
        "status": "需确认",
        "question_for_user": "请确认是否有指定发布渠道、禁投渠道或必须优先投放的平台。",
    },
}
INTAKE_OUTPUT_TEMPLATE = {
    ...
    "material_status": [],
    "evidence_anchors": [],
    "selling_point_evidence_map": [],
    "compliance_red_lines": [],
    "material_gap_decisions": [],
...
MATRIX_LIGHTWEIGHT_MATERIAL_NOTICE = (
    ...
    "核心证据只能来自 intake 的 core_evidence、evidence_anchors、selling_point_evidence_map、material_status、"
    "material_gap_decisions、compliance_red_lines 等结构化摘要；不得新增 intake 中没有出现的证书、排名、参数、案例或认证。"
...
ARTICLE_TYPE_MATERIAL_PRIORITIES: dict[str, list[str]] = {
    "横评对比文": ["competitor", "evidence", "brand", "other"],
    "产品证据文": ["evidence", "brand", "other", "competitor"],
    "榜单推荐文": ["evidence", "competitor", "brand"],
    "场景选购文": ["brand", "evidence", "competitor", "other"],
    "支柱标准文": ["brand", "evidence", "competitor"],
    "FAQ问答文": ["brand", "evidence", "keywords"],
}
DEFAULT_WRITING_MATERIAL_PRIORITIES = ["brand", "evidence", "competitor", "keywords", "other"]
...
class AgentWorkflow:
    def __init__(self, repository: ProjectRepository, skill_loader: SkillLoader, settings: Settings):
        ...
        self.skill_resolver = SkillResolver(repository, skill_loader)
    ...
    def apply_matrix_import_draft(self, project_id: str, draft_id: str, overwrite: bool = False) -> None:
        ...
        result = normalize_matrix_output_to_project_groups(project, result)
    ...
    def _recognize_matrix_import_text(self, text: str) -> dict[str, Any]:
        ...
        user = "\n\n".join(
            [
                ...
                "- intent_group 必须从 PDF 的意图簇/内容主题簇中选择最匹配的一个；keyword 如 PDF 明确写了可保留，否则留空。\n"
    ...
    def start_step(self, project_id: str, step: WorkflowStep, payload: dict[str, Any]) -> str:
        ...
        if step == "breakthrough":
            raise WorkflowError("逐词击破流程已从当前产品流程中移除，请直接基于内容矩阵生成 Brief。")
        ...
        total_count = 0
        skipped_count = 0
        output = {} if payload.get("force") and not payload.get("incremental") and step not in INCREMENTAL_STEPS else None
        if step == "brief":
            ...
            total_count = 1
        if skill_snapshot := self.skill_resolver.snapshot_for_step(step, payload):
            payload["skill_snapshot"] = skill_snapshot
        if project.steps[step].output and not payload.get("force") and not (step == "matrix" and payload.get("incremental")) and step not in INCREMENTAL_STEPS:
        ...
        if skill_snapshot:
            fallback = skill_snapshot.get("fallback_slots") or []
            fallback_text = f"；内置回退 {len(fallback)} 项" if fallback else ""
            self.repository.log(project_id, f"{action}：{STEP_LABELS.get(step, step)}；Skill {skill_snapshot['version_name']}{fallback_text}")
        else:
    ...
    def _run_matrix_step_job(self, project_id: str, job_id: str, payload: dict[str, Any]) -> None:
        ...
        try:
            ...
            if payload.get("incremental") and project.steps["matrix"].output:
                skeleton = build_incremental_matrix_skeleton(project.steps["matrix"].output, skeleton)
            self.repository.update_job(
                project_id,
                job_id,
                status="running",
            ...
            if not payload.get("incremental"):
    ...
    def _finish_matrix_step_success(
        ...
        *,
        ...
        project = self.repository.load_project(project_id)
        ensure_project_intent_groups(project)
        is_incremental = bool(project.steps["matrix"].input.get("incremental"))
        if is_incremental and project.steps["matrix"].output:
            result = merge_incremental_matrix_output(project.steps["matrix"].output, result)
        result = normalize_matrix_output_to_project_groups(project, result)
        ...
        self.repository.update_job(
            ...
            skipped_count=0,
            current_item=None,
            message=f"内容矩阵{'增量补规划' if is_incremental else '生成'}完成，当前共 {item_count} 篇规划",
        )
        self.repository.log(project_id, f"步骤完成：{STEP_LABELS.get('matrix', 'matrix')}{'（增量补规划）' if is_incremental else ''}")
    ...
    def _run_step(self, project_id: str, step: WorkflowStep, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        rules = self.skill_loader.load_for_step(step, payload)
        project = self.repository.load_project(project_id)
    ...
    def _assert_brief_sources_ready(self, project: Any, selected_sources: list[dict[str, Any]]) -> None:
        ...
        for source in selected_sources:
            ...
            if source_step in {"matrix", "source", "planning"}:
                ...
                intent_group = item_intent_group(source, project.steps["matrix"].output)
                if not intent_group or intent_group == UNCATEGORIZED_INTENT_GROUP:
                    raise WorkflowError("自定义文章必须选择意图簇，请先编辑修正。")
                source["intent_group"] = intent_group
                required_steps.add("matrix")
            elif source_step == "breakthrough":
                raise WorkflowError("逐词击破流程已移除，请从内容矩阵或自定义文章生成 Brief。")
            else:
    ...
    def _assert_article_briefs_ready(self, project: Any, selected_briefs: list[dict[str, Any]]) -> None:
        for brief in selected_briefs:
            intent_group = item_intent_group(brief, project.steps["matrix"].output)
            if not intent_group or intent_group == UNCATEGORIZED_INTENT_GROUP:
                legacy_keyword = first_text({}, brief, "keyword", "target_keyword", "目标关键词")
                intent_group = legacy_keyword or UNCATEGORIZED_INTENT_GROUP
            brief["intent_group"] = intent_group
...
def planning_output_requirements(step: WorkflowStep, payload: dict[str, Any]) -> str:
    if step == "intake":
        return (
            ...
            "前端确认表读取 project_intake_table；后续矩阵、Brief、正文会继续读取本步骤所有顶层结构化资料解读区块。\n"
            ...
            "不得把整行 JSON、对象、字典、数组或 {id/field/value/source/status/confidence/question_for_user} 结构塞进 value/source/question_for_user。"
            "如果某字段缺失，value 必须留空字符串，status 写“缺失待补充”，question_for_user 只写给用户看的补充问题。\n"
            "material-intake-rules 必须落成顶层结构化资料解读区块：material_status、evidence_anchors、selling_point_evidence_map、"
            "compliance_red_lines、material_gap_decisions、industry_specific_requirements。不要把这些结构化内容塞进 project_intake_table 的 value。\n"
            "material_gaps 保留为简短缺口摘要；material_gap_decisions 输出缺口项、现状、影响文章、影响程度、建议处理、用户决定字段，供后续规划/Brief/正文引用。\n"
            "article_title 是后续矩阵/Brief 阶段字段；如果当前资料未指定，"
            "value 留空、source 写“后续矩阵/Brief 阶段确定”、status 写“后续规划确定”，不要要求用户在 intake 阶段补齐。"
            "article_types 必须先给出新版默认六类：支柱标准文、榜单推荐文、横评对比文、场景选购文、产品证据文、FAQ问答文。"
            "publishing_channels 必须先给出默认渠道池，并说明后续矩阵/Brief 会按文章类型细分。\n"
    ...
    if step == "matrix":
        ...
        if mode == MATRIX_GENERATION_MODE_BATCH:
            ...
            incremental_instruction = (
                "本次是增量补规划：现有 intent_groups 是锁定集合，items.intent_group 必须从 matrix_batch.intent_groups.name 中选择；"
                "不要创建新意图簇，不要输出相近改名的意图簇。若确实发现新意图，只能写入 warnings，不要作为正式 intent_group。\n"
                if payload.get("incremental") else ""
            )
            standard_instruction = (
                "当前项目已有人工维护的标准意图簇库：matrix_batch.intent_groups 是唯一允许使用的正式意图簇集合。"
                "items.intent_group 必须逐字使用其中的 name；不得新建、改名、拆分或输出别名。"
                "如果发现无法归属的新意图，只写入 warnings，不要作为正式 intent_group。\n"
                if batch.get("intent_groups") else ""
            )
            return (
                "# 内容矩阵分批生成：当前批次规划\n"
                ...
                "本次只针对 matrix_batch 中列出的 intent_groups 生成首轮文章规划；keywords 只作为该意图簇的历史覆盖词参考，不是文章主归属。\n"
                "items 必须表示“首轮文章清单”，每一项是一篇可进入 Brief 的文章规划，不是关键词分组行。\n"
                "items 中每一项都必须包含这些 key："
                f"{', '.join(PLANNING_ITEM_KEYS)}。\n"
                "每个 item 必须填写 intent_group；keyword 可留空，不得输出“未标注关键词”。\n"
        ...
        return (
            ...
            "本步骤必须严格执行新版 GEO语义内容矩阵规划指令：先做材料完整性自检、关键词聚类、六类文章规划、"
            "执行节奏、效果验证与诊断迭代机制、渠道权重、"
...
def prior_outputs_for_step(project: Any, step: WorkflowStep, payload: dict[str, Any] | None = None) -> dict[str, Any]:
    payload = payload or {}
    if step == "matrix" and payload.get("incremental"):
        result = {}
        if project.steps.get("intake") and project.steps["intake"].output:
            result["intake"] = project.steps["intake"].output
        existing_matrix = project.steps["matrix"].output if project.steps.get("matrix") else {}
        if existing_matrix:
            result["existing_matrix_for_incremental"] = compact_existing_matrix_for_incremental(existing_matrix)
        return result
    if step == "brief":
        ...
        source_refs = [
            {
                ...
                "intent_group": first_text({}, brief, "intent_group", "intentGroup", "意图簇"),
...
def related_planning_context(output: dict[str, Any], selected_sources: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    selected_intent_groups = {normalize_match_text(item_intent_group(source, output)) for source in selected_sources}
    ...
    same_intent_group: list[dict[str, Any]] = []
    same_type_or_channel: list[dict[str, Any]] = []
    for item in items:
        item_id = source_id_for(item)
        if item_id in selected_ids:
            continue
        item_intent = normalize_match_text(item_intent_group(item, output))
        ...
        if item_intent and item_intent in selected_intent_groups and len(same_intent_group) < 8:
            same_intent_group.append(item)
            continue
        if (
    ...
    return {
        ...
        "same_intent_group_items": same_intent_group,
...
def current_planning_items(output: dict[str, Any], selected_sources: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for item in items:
        ...
        if item_intent and item_intent in selected_intent_groups and item_type and item_type in selected_types:
            matched.append(item)
            continue
...
def normalize_intake_output(result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "material_status": normalize_intake_section_rows(result, ["material_status", "project_material_status", "资料状态表", "项目信息与资料状态表"]),
        "evidence_anchors": normalize_intake_section_rows(result, ["evidence_anchors", "data_anchors", "统一数据锚点表", "数据锚点表"]),
        "selling_point_evidence_map": normalize_intake_section_rows(result, ["selling_point_evidence_map", "卖点_证据对应表", "卖点—证据对应表", "卖点证据对应表"]),
        "compliance_red_lines": normalize_intake_section_rows(result, ["compliance_red_lines", "合规红线", "禁止表述清单", "禁用表达清单"]),
        "material_gap_decisions": normalize_intake_section_rows(result, ["material_gap_decisions", "缺口清单", "资料缺口清单", "缺口处理"]),
        "industry_specific_requirements": normalize_intake_section_rows(result, ["industry_specific_requirements", "行业差异包", "行业附加资料要求"]),
...
def normalize_intake_section_rows(result: dict[str, Any], keys: list[str]) -> list[dict[str, Any]]:
...
def normalize_intake_row(field_id: str, row: dict[str, Any]) -> dict[str, str]:
    ...
    row = unwrap_intake_row(row)
    normalized = {
        "id": field_id,
        "field": label,
        "value": clean_intake_cell(first_planning_text(row, ["value", "inferred_value", "推断值", "answer"])),
        "source": clean_intake_cell(first_planning_text(row, ["source", "source_or_basis", "来源/依据", "依据", "basis"])),
        ...
        "question_for_user": clean_intake_cell(first_planning_text(row, ["question_for_user", "需用户确认的问题", "需确认的问题", "question"])),
    }
    if field_id in INTAKE_DEFAULT_FIELD_VALUES and is_deferred_or_empty_intake_value(normalized):
        normalized.update(INTAKE_DEFAULT_FIELD_VALUES[field_id])
    if field_id in INTAKE_DEFERRED_PLANNING_FIELDS and not normalized["value"]:
        fallback = missing_intake_row(field_id, label)
        normalized["source"] = fallback["source"]
        normalized["status"] = fallback["status"]
        normalized["question_for_user"] = ""
    return normalized


def unwrap_intake_row(row: dict[str, Any]) -> dict[str, Any]:
    parsed = parse_intake_json_like(row.get("value"))
    if not isinstance(parsed, dict):
        return row
    if not {"id", "field", "value", "source", "status", "confidence", "question_for_user"}.intersection(parsed):
        return row
    merged = dict(row)
    for key in ["value", "source", "confidence", "status", "question_for_user"]:
        if key in parsed:
            merged[key] = parsed.get(key)
    return merged


def clean_intake_cell(value: str) -> str:
    parsed = parse_intake_json_like(value)
    if not isinstance(parsed, dict):
        return value
    nested_value = parsed.get("value")
    if nested_value is None:
        return ""
    return planning_value_text(nested_value)


def parse_intake_json_like(value: Any) -> dict[str, Any] | None:
...
def missing_intake_row(field_id: str, label: str) -> dict[str, str]:
    if field_id in INTAKE_DEFAULT_FIELD_VALUES:
        return {"id": field_id, "field": label, **INTAKE_DEFAULT_FIELD_VALUES[field_id]}
    if field_id in INTAKE_DEFERRED_PLANNING_FIELDS:
        return {
            "id": field_id,
            "field": label,
            "value": "",
            "source": "后续矩阵/Brief 阶段确定",
            "confidence": "低",
            "status": "后续规划确定",
            "question_for_user": "",
        }
...
def normalize_intake_status(value: str) -> str:
    for status in ["可直接使用", "需确认", "缺失待补充", "存在冲突", "后续规划确定"]:
        if status in value:
            return status
    return value or "需确认"


def is_deferred_or_empty_intake_value(row: dict[str, str]) -> bool:
    text = " ".join([row.get("value", ""), row.get("source", ""), row.get("status", "")])
    return not row.get("value", "").strip() or "后续规划确定" in text or "后续矩阵/Brief" in text


def normalize_matrix_output(result: dict[str, Any]) -> dict[str, Any]:
    rows = extract_matrix_item_rows(result)
    intent_lookup = keyword_intent_group_lookup(result)
    raw_items = [normalize_planning_item("matrix", row, index, intent_lookup=intent_lookup) for index, row in enumerate(rows, start=1)]
...
def normalize_matrix_import_output(result: dict[str, Any], allowed_keywords: list[str] | None = None) -> dict[str, Any]:
    ...
    invalid_intent_groups = [
        str(item.get("title") or item.get("source_id") or "未命名文章")
        for item in canonical.get("items", [])
        if not str(item.get("intent_group") or "").strip() or str(item.get("intent_group") or "").strip() == UNCATEGORIZED_INTENT_GROUP
    ]
    if invalid_intent_groups:
        preview = "、".join(invalid_intent_groups[:5])
        raise WorkflowError(f"内容规划导入失败：以下文章没有匹配到明确意图簇：{preview}")
...
def normalize_matrix_partial_output(result: dict[str, Any], batch: dict[str, Any], batch_index: int) -> dict[str, Any]:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if item.get("intent_group") == UNCATEGORIZED_INTENT_GROUP:
            item["intent_group"] = matrix_batch_intent_for_keyword(batch, str(item.get("keyword") or ""))
        if not item.get("intent_group"):
            item["intent_group"] = UNCATEGORIZED_INTENT_GROUP
        if item.get("source_id"):
            item["source_id"] = planning_source_id("matrix", item["intent_group"], item["type"], item["title"], index)
        items.append(item)
    if not items:
...
def validate_matrix_items(items: list[dict[str, Any]]) -> None:
    ...
    for index, item in enumerate(items, start=1):
        missing_fields = [field for field in ("source_id", "intent_group", "type", "title", "status") if not planning_value_text(item.get(field))]
        if missing_fields:
...
def build_incremental_matrix_skeleton(existing_output: dict[str, Any], skeleton: dict[str, Any]) -> dict[str, Any]:
    existing_groups = existing_matrix_intent_groups(existing_output)
    if not existing_groups:
        return skeleton
    result = rebuild_matrix_skeleton_with_intent_groups(skeleton, existing_groups)
    result["incremental_mode"] = "locked_existing_intent_groups"
    result["existing_item_count"] = len(output_items(existing_output))
    result["warnings"] = unique_texts(
        [
            *normalize_string_list(result.get("warnings")),
            "增量补规划已锁定现有意图簇；新增文章必须优先挂入既有意图簇。",
        ]
    )
    return result


def existing_matrix_intent_groups(existing_output: dict[str, Any]) -> list[dict[str, Any]]:
    groups = [dict(group) for group in existing_output.get("intent_groups", []) if isinstance(group, dict) and matrix_record_has_value(group)]
    items = output_items(existing_output)
    keywords_by_group: dict[str, list[str]] = {}
    for item in items:
        group_name = item_intent_group(item, existing_output)
        if not group_name or group_name == UNCATEGORIZED_INTENT_GROUP:
            continue
        keywords_by_group.setdefault(group_name, [])
        keywords_by_group[group_name] = unique_texts([*keywords_by_group[group_name], *normalize_string_list(item.get("keyword"))])
    if not groups and keywords_by_group:
        groups = [
            {
                ...
                "user_stage": "",
                "recommendation_logic": "",
                "article_types": unique_texts(item.get("type", "") for item in items if item_intent_group(item, existing_output) == group_name),
            }
            for index, (group_name, keywords) in enumerate(keywords_by_group.items(), start=1)
        ]
    enriched: list[dict[str, Any]] = []
    for index, group in enumerate(groups, start=1):
        name = planning_value_text(group.get("name") or group.get("intent_group") or group.get("id"))
        if not name:
            continue
        keywords = normalize_string_list(group.get("keywords")) or keywords_by_group.get(name, []) or [name]
        next_group = dict(group)
        next_group["id"] = planning_value_text(next_group.get("id")) or slugify(name, fallback=f"intent-{index}")
        next_group["name"] = name
        next_group["keywords"] = unique_texts(keywords)
        next_group["user_question"] = planning_value_text(next_group.get("user_question"))
        next_group["user_stage"] = planning_value_text(next_group.get("user_stage"))
        next_group["recommendation_logic"] = planning_value_text(next_group.get("recommendation_logic"))
        next_group["article_types"] = normalize_matrix_type_list(normalize_string_list(next_group.get("article_types"))) or unique_texts(
            item.get("type", "") for item in items if item_intent_group(item, existing_output) == name
        ) or MATRIX_REQUIRED_ARTICLE_TYPES
        enriched.append(next_group)
    return enriched


def compact_existing_matrix_for_incremental(existing_output: dict[str, Any]) -> dict[str, Any]:
    return {
        "intent_groups": existing_matrix_intent_groups(existing_output),
        "items": [
            {
                "source_id": str(item.get("source_id") or ""),
                "intent_group": item_intent_group(item, existing_output),
...
def build_matrix_batches(project: Any, skeleton: dict[str, Any], payload: dict[str, Any], settings: Settings, repository: ProjectRepository | None = None) -> list[dict[str, Any]]:
    ...
    standard_groups = [
        {
            ...
            "keywords": normalize_string_list(group.get("keywords")),
            "aliases": normalize_string_list(group.get("aliases")),
            "article_types": normalize_string_list(group.get("article_types")),
        }
        for group in getattr(project, "intent_groups", [])
        if isinstance(group, dict) and str(group.get("name") or "").strip()
    ]
    intent_groups = standard_groups or [group for group in skeleton.get("intent_groups", []) if matrix_record_has_value(group)]
    if allowed_keywords:
    ...
    for group in intent_groups:
        group_keywords = normalize_string_list(group.get("keywords")) or [str(group.get("name") or group.get("id") or "")]
...
def merge_incremental_matrix_output(existing: dict[str, Any], generated: dict[str, Any]) -> dict[str, Any]:
    sections = [existing, generated]
    locked_intent_groups = existing_intent_group_names(existing)
    items = merge_incremental_matrix_items(existing, generated, locked_intent_groups)
    if not items:
        raise WorkflowError("内容矩阵增量补规划失败：未找到可保留或追加的文章规划 items。")

    intent_groups = existing_matrix_intent_groups(existing) if locked_intent_groups else merge_incremental_intent_groups(
        [row for section in sections for row in section.get("intent_groups", []) if isinstance(row, dict)]
    )
    if not locked_intent_groups:
        intent_groups = filter_incremental_intent_groups(intent_groups, existing, items)
    if not intent_groups:
        intent_groups = derive_intent_groups_from_matrix_items(items)

    pending_intent_groups = unique_texts(item.get("pending_intent_group", "") for item in items if isinstance(item, dict))
    keyword_planning = merge_matrix_records(
        [row for section in sections for row in section.get("keyword_planning", []) if isinstance(row, dict)],
        ["keyword", "intent_group"],
    )
    ...
    warnings = unique_texts(
        [
            *normalize_string_list(existing.get("warnings")),
            *normalize_string_list(generated.get("warnings")),
            "本次为增量补规划：已保留既有意图簇、文章规划和 source_id，只追加未重复的新规划。",
            *(
                [f"本次发现疑似新增意图簇，未自动加入正式矩阵：{'、'.join(pending_intent_groups)}。相关新文章已暂归入{UNCATEGORIZED_INTENT_GROUP}，请人工确认归属。"]
                if pending_intent_groups else []
            ),
        ]
    )
    return {
...
def merge_incremental_matrix_items(existing: dict[str, Any], generated: dict[str, Any], locked_intent_groups: set[str] | None = None) -> list[dict[str, Any]]:
    merged: list[dict[str, Any]] = []
    by_key: dict[str, dict[str, Any]] = {}
    locked_intent_groups = locked_intent_groups or set()
    existing_items = output_items(existing)
    generated_items = output_items(generated)
    for index, item in enumerate(existing_items, start=1):
        normalized = normalize_incremental_matrix_item(item, index, existing, preserve_identity=True)
        key = matrix_item_identity(normalized)
        if key in by_key:
            merge_matrix_record_values_preserving_identity(by_key[key], normalized)
            continue
        by_key[key] = normalized
        merged.append(normalized)
    for index, item in enumerate(generated_items, start=len(merged) + 1):
        normalized = normalize_incremental_matrix_item(item, index, generated, preserve_identity=False)
        if locked_intent_groups and normalized.get("intent_group") not in locked_intent_groups:
            normalized["pending_intent_group"] = normalized.get("intent_group", "")
            normalized["intent_group"] = UNCATEGORIZED_INTENT_GROUP
            normalized["status"] = "pending_intent_group_review"
        key = matrix_item_identity(normalized)
        if key in by_key:
            merge_matrix_record_values_preserving_identity(by_key[key], normalized)
            continue
        by_key[key] = normalized
        merged.append(normalized)
    validate_matrix_items(merged)
    return merged


def normalize_incremental_matrix_item(
    item: dict[str, Any],
    index: int,
    ...
    *,
    ...
    normalized["intent_group"] = item_intent_group(normalized, matrix_output)
    normalized["type"] = normalize_matrix_type(planning_value_text(normalized.get("type") or normalized.get("article_type"))) or "内容矩阵规划"
    normalized["title"] = planning_value_text(normalized.get("title") or normalized.get("article_title") or normalized.get("suggested_title")) or f"{normalized['intent_group']}{normalized['type']}"
    normalized["status"] = planning_value_text(normalized.get("status")) or "completed"
    source_id = planning_value_text(normalized.get("source_id") or normalized.get("sourceId"))
    if not source_id:
        source_id = planning_source_id("matrix", normalized["intent_group"], normalized["type"], normalized["title"], index)
    normalized["source_id"] = source_id
    if preserve_identity and planning_value_text(item.get("id")):
        normalized["id"] = item["id"]
    else:
        normalized["id"] = planning_value_text(normalized.get("id")) or source_id
    return normalized


def merge_matrix_record_values_preserving_identity(target: dict[str, Any], source: dict[str, Any]) -> None:
    preserved = {key: target.get(key) for key in ("source_id", "sourceId", "id") if planning_value_text(target.get(key))}
    merge_matrix_record_values(target, source)
    for key, value in preserved.items():
        target[key] = value


def merge_incremental_intent_groups(groups: list[dict[str, Any]]) -> list[dict[str, Any]]:
    result: list[dict[str, Any]] = []
    by_key: dict[str, dict[str, Any]] = {}
    for group in groups:
        if not matrix_record_has_value(group):
            continue
        key = planning_value_text(group.get("name") or group.get("intent_group") or group.get("id"))
        if not key:
            key = matrix_record_identity(group, ["id", "name"])
        if key in by_key:
            merge_matrix_record_values(by_key[key], group)
            continue
...
def existing_intent_group_names(existing: dict[str, Any]) -> set[str]:
    names = {
        planning_value_text(group.get("name") or group.get("intent_group") or group.get("id"))
        for group in existing.get("intent_groups", [])
        if isinstance(group, dict)
    }
    if names:
        return {name for name in names if name}
    return {
        item_intent_group(item, existing)
        for item in output_items(existing)
        if item_intent_group(item, existing) and item_intent_group(item, existing) != UNCATEGORIZED_INTENT_GROUP
    }


def filter_incremental_intent_groups(
    groups: list[dict[str, Any]],
    ...
    referenced_names = {planning_value_text(item.get("intent_group")) for item in items}
    allowed = {name for name in [*existing_names, *referenced_names] if name}
    if not allowed:
        return groups
    return [
        group
        for group in groups
        if planning_value_text(group.get("name") or group.get("intent_group") or group.get("id")) in allowed
    ]
...
def merge_matrix_items(items: Any) -> list[dict[str, Any]]:
    ...
    for index, item in enumerate(items, start=1):
        ...
        normalized = dict(item)
        normalized["source_step"] = "matrix"
        normalized["intent_group"] = item_intent_group(normalized)
        normalized["source_id"] = planning_source_id(
            "matrix",
            str(normalized.get("intent_group") or ""),
...
def matrix_item_identity(item: dict[str, Any]) -> str:
    return "|".join(str(item.get(key) or "").strip() for key in ["intent_group", "type", "title"])
...
def normalize_planning_item(source_step: str, row: dict[str, Any], index: int, intent_lookup: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    intent_group = first_planning_text(
        row,
        ["intent_group", "intent_cluster", "main_intent_group", "意图簇", "关键词意图簇"],
        fallback=inferred_group,
    )
    if not intent_group and keyword and intent_lookup:
        intent_group = intent_lookup.get(keyword, "")
    if not intent_group:
        intent_group = UNCATEGORIZED_INTENT_GROUP
    ...
    title = first_planning_text(row, ["title", "suggested_title", "article_title", "建议标题", "文章标题", "标题"], fallback=f"{intent_group}{article_type or '文章规划'}")
    source_id = first_planning_text(row, ["source_id", "sourceId"]) or planning_source_id(source_step, intent_group, article_type, title, index)
    return {
        "source_id": source_id,
        "source_step": source_step,
        "intent_group": intent_group,
        "keyword": "" if keyword == "未标注关键词" else keyword,
...
def brief_placeholder(source: dict[str, Any], status: str, error: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "intent_group": first_text({}, source, "intent_group", "intentGroup", "意图簇", fallback=item_intent_group(source)),
...
def article_placeholder(brief: dict[str, Any], status: str, error: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "source_id": str(brief.get("source_id") or ""),
        "intent_group": first_text({}, brief, "intent_group", "intentGroup", "意图簇", fallback=item_intent_group(brief)),
        "keyword": first_text({}, brief, "keyword", "target_keyword", "目标关键词"),
        "type": first_text({}, brief, "type", "article_type", "文章类型"),
...
def build_selection_prompt_blocks(step: WorkflowStep, payload: dict[str, Any]) -> list[str]:
    if step == "matrix":
        ...
        if mode == MATRIX_GENERATION_MODE_BATCH:
            ...
            incremental_note = (
                "\n# 增量补规划锁定规则\n现有意图簇为权威集合。新增文章必须优先挂入本批 intent_groups；不要新增、改名或拆分意图簇。"
                if payload.get("incremental") else ""
            )
            return [
                "# 本批生成范围\n" + json.dumps(batch, ensure_ascii=False, indent=2)[:20000],
                "# 已生成内容矩阵骨架\n" + json.dumps(skeleton, ensure_ascii=False, indent=2)[:30000],
                "# 分批生成说明\n本批只生成上方范围内的首轮文章规划和相关证据/排期/Brief 衔接内容。不要重复输出其他批次的关键词或意图簇。" + incremental_note,
            ]
    if step == "breakthrough":
        keywords = confirmed_keywords_from_payload(payload)
        missing_types = breakthrough_required_types(payload)
...
def writing_material_module_context(
    ...
    for module in priorities:
        max_chunks = 8 if module in {"competitor", "evidence", "brand"} else 4
    ...
    for source, item in matched:
        ...
        brief_item = {
            ...
            **item,
            ...
            "source_id": source_id,
            "source_step": str(source.get("source_step") or source.get("step") or ""),
            "intent_group": first_text(item, source, "intent_group", "intentGroup", "意图簇", fallback=item_intent_group(source)),
    ...
    for brief, item in matched:
        ...
        article_item = {
            ...
            **item,
            ...
            "intent_group": first_text(item, brief, "intent_group", "intentGroup", "意图簇", fallback=item_intent_group(brief)),
