...
from app.models.schemas import (
    ...
    HealthResponse,
    IntentGroupMergeRequest,
    IntentGroupUpdateRequest,
...
def health(
    ...
    return HealthResponse(
        ...
        active_skill_name="按文章类型分别选择",
        active_skill_id="per-slot",
        publishing_frontend_url=settings.publishing_frontend_url,
    )


@router.get("/skills")
def list_skills(repository: ProjectRepository = Depends(get_repository)):
    return repository.skill_catalog()


@router.post("/skills/slots/{article_type}/{stage}")
async def upload_skill_slot(
    article_type: str,
    stage: str,
    ...
    try:
        return repository.upload_skill_candidate(article_type, stage, file.filename or "rule.md", await file.read())
...
@router.post("/skills/slots/{article_type}/{stage}/candidates/{candidate_id}/activate")
def activate_skill_candidate(article_type: str, stage: str, candidate_id: str, repository: ProjectRepository = Depends(get_repository)):
    try:
        return repository.activate_skill_candidate(article_type, stage, candidate_id)
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
@router.delete("/skills/slots/{article_type}/{stage}/candidates/{candidate_id}")
def delete_skill_candidate(article_type: str, stage: str, candidate_id: str, repository: ProjectRepository = Depends(get_repository)):
    try:
        return repository.delete_skill_candidate(article_type, stage, candidate_id)
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
@router.get("/projects/{project_id}/intent-groups")
def list_intent_groups(project_id: str, repository: ProjectRepository = Depends(get_repository)):
    try:
        return {"intent_groups": repository.list_intent_groups(project_id)}
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc


@router.post("/projects/{project_id}/intent-groups/rebuild-from-archive")
def rebuild_intent_groups(project_id: str, repository: ProjectRepository = Depends(get_repository)):
    try:
        return {"project": project_payload(repository.rebuild_intent_groups(project_id), repository)}
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc


@router.post("/projects/{project_id}/intent-groups")
def create_project_intent_group(
    project_id: str,
    payload: IntentGroupUpdateRequest,
    repository: ProjectRepository = Depends(get_repository),
):
    try:
        return {"project": project_payload(repository.create_intent_group(project_id, payload.model_dump(exclude_unset=True)), repository)}
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
@router.patch("/projects/{project_id}/intent-groups/{group_id}")
def update_project_intent_group(
    project_id: str,
    group_id: str,
    payload: IntentGroupUpdateRequest,
    repository: ProjectRepository = Depends(get_repository),
    ...
    try:
        return {"project": project_payload(repository.update_intent_group(project_id, group_id, payload.model_dump(exclude_unset=True)), repository)}
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
@router.post("/projects/{project_id}/intent-groups/{group_id}/merge")
def merge_project_intent_group(
    project_id: str,
    group_id: str,
    payload: IntentGroupMergeRequest,
    repository: ProjectRepository = Depends(get_repository),
):
    try:
        return {"project": project_payload(repository.merge_intent_group(project_id, group_id, payload.source_group_ids), repository)}
    except FileNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
