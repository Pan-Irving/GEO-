...
from app.storage.mysql_schema import (
    ...
    writing_projects,
    writing_app_settings,
    writing_skill_slots,
    writing_skill_versions,
    writing_skill_candidates,
    writing_skill_active_slots,
    writing_steps,
)
from app.storage.repository import ProjectRepository, normalize_blocked_step_states
from app.services.skill_registry import SKILL_ARTICLE_TYPES, SKILL_STAGES, normalize_catalog, slot_key
...
class MySQLProjectRepository(ProjectRepository):
    ...
    def _load_skill_catalog_data(self) -> dict[str, Any]:
        with self.engine.connect() as conn:
            candidates = conn.execute(select(writing_skill_candidates)).mappings().all()
            active_rows = conn.execute(select(writing_skill_active_slots)).mappings().all()
        catalog = normalize_catalog(None)
        for row in candidates:
            key = slot_key(row["article_type"], row["stage"])
        ...
        for row in active_rows:
            catalog["slots"][slot_key(row["article_type"], row["stage"])]["active_id"] = row["candidate_id"]
    ...
    def _save_skill_catalog_data(self, catalog: dict[str, Any]) -> None:
        ...
        with self.engine.begin() as conn:
            conn.execute(delete(writing_skill_candidates))
            conn.execute(delete(writing_skill_active_slots))
            candidate_rows = []
            active_rows = []
            for key, slot in normalized["slots"].items():
                article_type, stage = key.rsplit(":", 1)
                active_rows.append(
                    {"article_type": article_type, "stage": stage, "candidate_id": slot["active_id"], "updated_at": utc_now()}
                )
                for candidate in slot["candidates"]:
                    if candidate.get("id") == "builtin":
                        continue
                    candidate_rows.append(
                        {
                            ...
                            "stored_name": candidate.get("stored_name") or "",
                            "sha256": candidate.get("sha256") or "",
                            "uploaded_at": candidate.get("uploaded_at") or utc_now(),
                        }
                    )
            if candidate_rows:
                conn.execute(insert(writing_skill_candidates), candidate_rows)
            conn.execute(insert(writing_skill_active_slots), active_rows)
