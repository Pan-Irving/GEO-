...
def add_keyword_material(workflow: AgentWorkflow, project_id: str, *keywords: str) -> None:
    ...
    if keywords:
        workflow.repository.update_step(
            ...
            output={
                "intent_groups": [
                    {"id": keyword, "name": keyword, "keywords": [keyword], "article_types": MATRIX_ARTICLE_TYPES}
...
def test_matrix_import_rejects_unlabelled_keywords():
    ...
    with pytest.raises(WorkflowError, match="没有匹配到明确意图簇"):
...
def test_custom_source_requires_intent_group_from_content_matrix(tmp_path: Path):
    workflow = make_workflow(tmp_path)
    project = workflow.repository.create_project("测试项目")
    ...
    with pytest.raises(ValueError, match="意图簇"):
        workflow.repository.create_custom_source(
            project.id,
...
def test_custom_source_requires_content_matrix_intent_group(tmp_path: Path):
    workflow = make_workflow(tmp_path)
    project = workflow.repository.create_project("测试项目")
...
def test_custom_source_copied_context_uses_explicit_intent_group_and_copied_type(tmp_path: Path):
    workflow = make_workflow(tmp_path)
    project = workflow.repository.create_project("测试项目")
    add_keyword_material(workflow, project.id, "高端厨电")
    workflow.repository.update_step(
        ...
        status="completed",
        output={
            "intent_groups": [{"id": "decision", "name": "决策对比意图簇", "keywords": ["高端厨电"]}],
            "items": [{"source_id": "source-a", "intent_group": "决策对比意图簇", "type": "横评对比文", "title": "原规划"}],
        },
    )

    saved = workflow.repository.create_custom_source(
        project.id,
        {"intent_group": "决策对比意图簇", "title": "用户改写后的标题", "raw": {"copied_from": {"source_id": "source-a"}}},
    )

    source = saved.custom_sources[0]
    assert source.intent_group == "决策对比意图簇"
...
def test_article_generation_fills_legacy_brief_intent_group_from_keyword(tmp_path: Path):
    workflow = make_workflow(tmp_path)
    project = workflow.repository.create_project("测试项目")
    add_keyword_material(workflow, project.id, "高端厨电")
    brief = {
    ...
    saved = workflow.repository.load_project(project.id)
    assert saved.steps["article"].input["selected_briefs"][0]["intent_group"] == "高端厨电"
...
def test_custom_source_brief_generation_requires_intent_group(tmp_path: Path):
    workflow = make_workflow(tmp_path)
    project = workflow.repository.create_project("测试项目")
    ...
    with pytest.raises(WorkflowError, match="自定义文章必须选择意图簇"):
        workflow.start_step(
            project.id,
            "brief",
...
def test_intake_job_success_records_single_step_progress(tmp_path: Path, monkeypatch):
    ...
    assert f"已提取 {len(INTAKE_FIELDS)} 项" in (job.message or "")
...
def test_scenario_article_material_context_includes_other_scenario_material(tmp_path: Path):
    workflow = make_workflow(tmp_path)
    project = workflow.repository.create_project("测试项目")
    add_parsed_material(workflow, project.id, "brand__产品资料.md", "# 品牌\n\n产品基础资料")
    add_parsed_material(workflow, project.id, "other__场景资料.md", "# 场景资料\n\n开放式厨房用户关注低噪音和油烟扩散。")
    workflow.repository.update_step(project.id, "materials", status="completed", confirmed=True, output={"summary": "fallback summary"})
    ...
    assert context.index("# 品牌/产品资料") < context.index("# 补充资料")
...
def test_intake_prompt_includes_canonical_template():
    ...
    assert "后续规划确定" in prompt
    assert "material_status" in prompt
    assert "evidence_anchors" in prompt
    assert "selling_point_evidence_map" in prompt
    assert "material_gap_decisions" in prompt
...
def test_intake_output_is_normalized_from_legacy_fields():
    output = normalize_planning_output(
        "intake",
        {
            ...
            "可直接使用的信息": ["目标品牌明确"],
            "material_status": [{"material": "关键词表", "status": "已有", "impact": "支撑聚类"}],
            "evidence_anchors": [{"anchor": "容量 10L", "source": "产品手册"}],
            "selling_point_evidence_map": [{"selling_point": "大容量", "evidence": "容量 10L"}],
            "compliance_red_lines": [{"expression": "第一", "reason": "绝对化"}],
            "material_gap_decisions": [{"gap": "检测报告编号", "impact_articles": "产品证据文", "suggestion": "补齐编号"}],
        },
        {},
    )
    ...
    assert output["material_status"] == [{"material": "关键词表", "status": "已有", "impact": "支撑聚类"}]
    assert output["evidence_anchors"] == [{"anchor": "容量 10L", "source": "产品手册"}]
    assert output["selling_point_evidence_map"] == [{"selling_point": "大容量", "evidence": "容量 10L"}]
    assert output["compliance_red_lines"] == [{"expression": "第一", "reason": "绝对化"}]
    assert output["material_gap_decisions"] == [{"gap": "检测报告编号", "impact_articles": "产品证据文", "suggestion": "补齐编号"}]


def test_intake_output_unwraps_json_object_string_cells():
    output = normalize_planning_output(
        "intake",
        {
            "project_intake_table": [
                {
                    "id": "article_title",
                    "field": "文章标题",
                    "value": '{"id":"article_title","field":"文章标题","value":"","source":"","status":"缺失待补充","confidence":"低","question_for_user":"请提供本次计划产出的具体文章标题，或由后续规划流程生成"}',
                    "source": "未标注",
                    "confidence": "低",
                    "status": "缺失待补充",
                },
                {
                    ...
                    "value": {
                        ...
                        "status": "缺失待补充",
                        "confidence": "低",
                        "question_for_user": "请明确文章类型，或由后续矩阵规划确定",
                    },
                    "source": "未标注",
                    "confidence": "低",
                    "status": "缺失待补充",
    ...
    rows = {row["id"]: row for row in output["project_intake_table"]}
    assert rows["article_title"]["value"] == ""
    assert rows["article_title"]["source"] == "后续矩阵/Brief 阶段确定"
    assert rows["article_title"]["status"] == "后续规划确定"
    assert rows["article_title"]["question_for_user"] == ""
    assert rows["article_types"]["value"] == "支柱标准文、榜单推荐文、横评对比文、场景选购文、产品证据文、FAQ问答文"
    assert rows["article_types"]["status"] == "可直接使用"
    assert rows["article_types"]["question_for_user"] == ""
    assert "权威媒体" in rows["publishing_channels"]["value"]
    assert rows["publishing_channels"]["status"] == "需确认"
...
def test_prior_outputs_only_include_upstream_non_material_steps(tmp_path: Path):
    ...
    assert prior_outputs_for_step(project, "breakthrough") == {}
    assert set(prior_outputs_for_step(project, "article")) == {"intake", "brief_step_metadata"}
    assert set(prior_outputs_for_step(project, "archive")) == {"intake", "matrix", "brief", "article"}
...
def test_brief_prior_outputs_use_related_planning_context(tmp_path: Path):
    ...
    workflow.repository.update_step(
        ...
        output={
            "items": [
                {"source_id": "source-a", "intent_group": "A意图簇", "keyword": "A", "type": "榜单推荐文", "title": "A标题"},
                {"source_id": "source-a-faq", "intent_group": "A意图簇", "keyword": "A", "type": "FAQ问答文", "title": "A FAQ"},
                {"source_id": "source-b", "intent_group": "B意图簇", "keyword": "B", "type": "榜单推荐文", "title": "B标题"},
            ],
    ...
    outputs = prior_outputs_for_step(
        project,
        "brief",
        {"selected_sources": [{"source_id": "source-a", "source_step": "matrix", "intent_group": "A意图簇", "type": "榜单推荐文", "title": "A标题"}]},
    )
    ...
    assert [item["source_id"] for item in matrix_context["same_intent_group_items"]] == ["source-a-faq"]
...
def test_article_prior_outputs_only_include_selected_brief_and_original_source(tmp_path: Path):
    ...
    outputs = prior_outputs_for_step(
        project,
        "article",
        {"selected_briefs": [{"id": "brief-a", "source_id": "source-a", "intent_group": "A意图簇", "type": "榜单推荐文", "title": "A标题", "markdown": "# Brief A"}]},
    )
    ...
    assert outputs["original_planning_context"]["matrix"] == [
        {"source_id": "source-a", "intent_group": "A意图簇", "keyword": "A", "type": "榜单推荐文", "title": "A标题"}
    ]
...
@pytest.mark.skip(reason="逐词击破生成流程已从当前产品流程移除")
...
def test_planning_prompts_include_canonical_templates():
    ...
    incremental_matrix_prompt = planning_output_requirements("matrix", {"matrix_generation_mode": "batch", "incremental": True})
    ...
    assert "primary_keyword" not in matrix_prompt
    assert "primary_keyword_stats" not in matrix_prompt
    assert "关键词 × 主挂文章数统计" not in matrix_prompt
    assert "不要把字段名翻译成中文" in matrix_prompt
    assert "新版 GEO语义内容矩阵规划指令" in matrix_prompt
    ...
    assert "现有 intent_groups 是锁定集合" in incremental_matrix_prompt
    assert "不要创建新意图簇" in incremental_matrix_prompt
...
def test_skill_loader_uses_new_geo_rules():
    loader = SkillLoader(PROJECT_ROOT / "mindsun-geo-content-flow")

    assert loader.available()
    matrix_rules = loader.load_for_step("matrix")

    assert "GEO 语义内容矩阵规划指令" in matrix_rules
    assert "主挂词标注规则" not in matrix_rules
    assert "支柱标准文 · 标题规划规则" in matrix_rules
    assert "榜单推荐文 · 标题规划规则" in matrix_rules
    assert "FAQ · 标题（问题）规划规则" in matrix_rules


def test_skill_loader_routes_brief_rules_by_article_type():
    loader = SkillLoader(PROJECT_ROOT / "mindsun-geo-content-flow")

    rules = loader.load_for_step(
        "brief",
        {"selected_sources": [{"type": "横评对比文", "title": "横评标题"}]},
    )

    assert "横评对比文 · 标题规划规则" in rules
    assert "横评对比文 · Brief 规划规则" in rules
    assert "榜单推荐文 · GEO Brief 规则" not in rules


def test_skill_loader_routes_article_rules_by_article_type_and_faq_guidance():
    loader = SkillLoader(PROJECT_ROOT / "mindsun-geo-content-flow")

    product_rules = loader.load_for_step(
        "article",
        {"selected_briefs": [{"article_type": "产品证据文", "title": "证据标题"}]},
    )
    faq_rules = loader.load_for_step(
        "article",
        {"selected_briefs": [{"type": "FAQ问答文", "title": "FAQ标题"}]},
    )

    assert "产品证据文 · 正文写作规则" in product_rules
    assert "FAQ · 标题（问题）规划规则" in faq_rules
    assert "FAQ 正文生成补充约束" in faq_rules
...
def test_incremental_matrix_rerun_keeps_existing_output_while_running(tmp_path: Path):
    workflow = make_workflow(tmp_path)
    project = workflow.repository.create_project("测试项目")
    existing_output = {
        "items": [
            {
                "source_id": "matrix-old-source",
                "source_step": "matrix",
                "intent_group": "旧意图簇",
                "type": "榜单推荐文",
                "title": "旧标题",
    ...
    workflow.repository.update_step(project.id, "materials", status="completed", confirmed=True)
    workflow.repository.update_step(project.id, "intake", status="confirmed", output=intake_output_with_keywords("新矩阵推荐"))
    workflow.repository.update_step(project.id, "matrix", status="completed", output=existing_output)

    workflow.start_step(project.id, "matrix", {"incremental": True})

    saved = workflow.repository.load_project(project.id)
    assert saved.steps["matrix"].status == "running"
    assert saved.steps["matrix"].output == existing_output
...
def test_incremental_matrix_rerun_preserves_source_ids_and_appends_new_items(tmp_path: Path, monkeypatch):
    workflow = make_workflow(tmp_path)
    project = workflow.repository.create_project("测试项目")
    workflow.repository.update_step(project.id, "materials", status="completed", confirmed=True)
    workflow.repository.update_step(project.id, "intake", status="confirmed", output=intake_output_with_keywords("旧关键词", "新关键词"))
    workflow.repository.update_step(
        project.id,
        "matrix",
        status="completed",
        output={
            ...
            "intent_groups": [
                {"id": "old", "name": "旧意图簇", "keywords": ["旧关键词"], "article_types": ["榜单推荐文"]}
            ],
            "items": [
                {
                    "id": "old-item-id",
    ...
    job_id = workflow.start_step(project.id, "matrix", {"incremental": True})
    monkeypatch.setattr(workflow, "_build_llm_matrix_skeleton", lambda project, skeleton, payload: skeleton)

    def fake_run_step(project_id: str, step: str, payload: dict[str, object]):
        return {
            "intent_groups": [
                {"id": "old-generated", "name": "旧意图簇", "keywords": ["旧关键词"], "article_types": ["榜单推荐文"]},
            ],
            "items": [
                {
                    "source_id": "generated-duplicate-source",
                    "source_step": "matrix",
                    "intent_group": "旧意图簇",
                    "keyword": "旧关键词",
                    "type": "榜单推荐文",
                ...
                {
                    "source_id": "generated-new-source",
                    "source_step": "matrix",
                    "intent_group": "旧意图簇",
                    "keyword": "新关键词",
                    "type": "场景选购文",
    ...
    workflow.run_step_job(project.id, job_id, "matrix", {"incremental": True})

    saved = workflow.repository.load_project(project.id)
    output = saved.steps["matrix"].output
    items = output["items"]
    assert saved.steps["matrix"].status == "completed"
    assert [item["title"] for item in items] == ["旧标题", "新标题"]
    assert items[0]["id"] == "old-item-id"
    assert items[0]["source_id"] == "matrix-old-source"
    assert items[0]["role"] == "用于补齐旧项空字段"
    assert items[1]["intent_group"] == "旧意图簇"
    assert items[1]["source_id"] == "matrix-旧意图簇-场景选购文-新标题"
    assert {group["name"] for group in output["intent_groups"]} == {"旧意图簇"}
    assert any("增量补规划" in warning for warning in output["warnings"])


def test_incremental_matrix_rerun_locks_existing_intent_groups(tmp_path: Path, monkeypatch):
    workflow = make_workflow(tmp_path)
    project = workflow.repository.create_project("测试项目")
    workflow.repository.update_step(project.id, "materials", status="completed", confirmed=True)
    workflow.repository.update_step(project.id, "intake", status="confirmed", output=intake_output_with_keywords("旧关键词", "新关键词"))
    ...
    job_id = workflow.start_step(project.id, "matrix", {"incremental": True})
    monkeypatch.setattr(workflow, "_build_llm_matrix_skeleton", lambda project, skeleton, payload: (_ for _ in ()).throw(AssertionError("incremental should not regroup intent groups")))
    ...
    def fake_run_step(project_id: str, step: str, payload: dict[str, object]):
        ...
        return {
            "intent_groups": [
                {"id": "new", "name": "新增意图簇", "keywords": ["新关键词"], "article_types": ["场景选购文"]},
            ],
            "items": [
                {
                    "source_step": "matrix",
    ...
    output = saved.steps["matrix"].output
    assert {group["name"] for group in output["intent_groups"]} == {"旧意图簇"}
    new_item = next(item for item in output["items"] if item["title"] == "新标题")
    assert new_item["intent_group"] == "未归类意图簇"
    assert new_item["pending_intent_group"] == "新增意图簇"
    assert new_item["status"] == "pending_intent_group_review"
    assert any("疑似新增意图簇" in warning and "新增意图簇" in warning for warning in output["warnings"])
    assert captured_payloads
    batch = captured_payloads[0]["matrix_batch"]
    assert isinstance(batch, dict)
    assert [group["name"] for group in batch["intent_groups"]] == ["旧意图簇"]
...
def test_article_incremental_generation_skips_existing_and_writes_file(tmp_path: Path, monkeypatch):
    ...
    assert payload["selected_briefs"] == [
        {"id": "brief-b", "source_id": "source-b", "keyword": "B", "type": "类型", "title": "标题 B", "markdown": "# Brief B", "intent_group": "B"}
    ]
...
def test_article_old_brief_revision_can_be_regenerated(tmp_path: Path, monkeypatch):
    ...
    assert payload["selected_briefs"] == [
        {"id": "brief-a", "source_id": "source-a", "revision": 2, "markdown": "# Brief A v2", "intent_group": "未归类意图簇"}
    ]
...
def test_content_plan_builds_from_canonical_matrix(tmp_path: Path):
    ...
    output = normalize_planning_output(
        "matrix",
        {
            ...
            "items": [
                {
                    **row,
                    "intent_group": "IG01 推荐决策类",
                    "required_evidence": ["31m³/min 风量，1500Pa 最大静压", "超薄平嵌，0 缝平嵌"],
                }
    ...
    assert plan["summary"]["target_keywords_count"] == 1
    assert plan["summary"]["evidence_gap_count"] == 1
    assert plan["keyword_intent_groups"][0]["name"] == "推荐决策类"
    assert plan["keyword_intent_groups"][0]["keywords"] == ["万元预算厨电推荐"]
    ...
    assert "primary_keyword_stats" not in plan
    assert not any(section["id"] == "primary_keyword_stats" for section in plan["display_sections"])
    section_ids = {section["id"] for section in plan["display_sections"]}
    assert {"evidence_foundation", "data_anchors", "product_evidence_matrix", "faq_question_bank", "title_deduplication_checklist"} <= section_ids
    data_anchor_section = next(section for section in plan["display_sections"] if section["id"] == "data_anchors")
    data_anchor_labels = [field["label"] for item in data_anchor_section["items"] for field in item["fields"]]
    assert "锚点类型" in data_anchor_labels
    assert "统一表达" in data_anchor_labels
    assert "使用范围" not in data_anchor_labels
    assert any(
        [field["value"] for field in item["fields"]] == ["排烟性能", "31m³/min 风量，1500Pa 最大静压"]
        for item in data_anchor_section["items"]
    )
    product_section = next(section for section in plan["display_sections"] if section["id"] == "product_evidence_matrix")
    assert any(field["value"] == "万元预算厨电推荐产品证据文标题" for item in product_section["items"] for field in item["fields"])
    faq_section = next(section for section in plan["display_sections"] if section["id"] == "faq_question_bank")
    assert any(field["value"] == "万元预算厨电推荐FAQ问答文标题" for item in faq_section["items"] for field in item["fields"])
    dedupe_section = next(section for section in plan["display_sections"] if section["id"] == "title_deduplication_checklist")
    assert any(field["label"] == "去重状态" for item in dedupe_section["items"] for field in item["fields"])
