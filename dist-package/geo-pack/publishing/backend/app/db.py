...
MATRIX_ARTICLE_TYPES = {"榜单推荐文", "横评对比文", "支柱标准文", "场景选购文"}
...
article_snapshots = Table(
    ...
    Index("idx_article_project_active_sort", "project_id", "active", "intent_group", "article_type"),
    Index("idx_article_intent_group_type", "intent_group", "article_type"),
...
assignments = Table(
    ...
    Column("user_id", String(64), ForeignKey("users.id", ondelete="CASCADE"), nullable=False),
    Column("project_id", String(191), nullable=False),
    Column("intent_group_ids_json", Text, nullable=False, default="[]"),
    Column("intent_groups_json", Text, nullable=False, default="[]"),
...
class PublishingStore:
    ...
    def migrate_schema(self) -> None:
        with self.engine.begin() as conn:
            ensure_column(conn, "article_snapshots", "intent_group_id", "VARCHAR(120) NOT NULL DEFAULT ''")
            ensure_column(conn, "article_snapshots", "intent_group", "VARCHAR(255) NOT NULL DEFAULT ''")
            ensure_column(conn, "assignments", "intent_group_ids_json", "TEXT NULL")
            ensure_column(conn, "assignments", "intent_groups_json", "TEXT NULL")
        ...
        with self.engine.begin() as conn:
            ...
            for article in articles:
                ...
                values = {
                    ...
                    "intent_group_id": clean(article.get("intent_group_id")),
                    "intent_group": clean(article.get("intent_group")) or clean(article.get("keyword")) or "未归类意图簇",
    ...
    def create_assignment(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        intent_groups = clean_list(payload.get("intent_groups")) or clean_list(payload.get("keywords"))
        intent_group_ids = clean_list(payload.get("intent_group_ids"))
        ...
        with self.engine.begin() as conn:
            conn.execute(
                insert(assignments).values(
                    ...
                    intent_group_ids_json=dumps(intent_group_ids),
                    intent_groups_json=dumps(intent_groups),
    ...
    def inventory(self, project_id: str, user: dict[str, Any]) -> dict[str, Any]:
        ...
        for article in articles:
            if article["article_type"] not in MATRIX_ARTICLE_TYPES:
                continue
            key = f"{article['intent_group']}\u0001{article['article_type']}"
            cell = matrix.setdefault(
                key,
                {
                    "intent_group_id": article.get("intent_group_id", ""),
                    "intent_group": article["intent_group"],
    ...
    def visible_articles(self, project_id: str, user: dict[str, Any]) -> list[dict[str, Any]]:
        with self.engine.connect() as conn:
            rows = conn.execute(
                ...
                .order_by(article_snapshots.c.intent_group, article_snapshots.c.article_type, article_snapshots.c.title)
            ).mappings().all()
        ...
        with self.engine.connect() as conn:
            query = (
                select(
                    ...
                    article_snapshots.c.project_id,
                    article_snapshots.c.intent_group,
...
def article_row(row: RowMapping) -> dict[str, Any]:
    data = dict(row)
    data["intent_group_id"] = clean(data.get("intent_group_id"))
    data["intent_group"] = clean(data.get("intent_group")) or clean(data.get("keyword")) or "未归类意图簇"
...
def assignment_row(row: RowMapping) -> dict[str, Any]:
    data = dict(row)
    data["intent_group_ids"] = loads(data.pop("intent_group_ids_json", "[]"), [])
    intent_groups = loads(data.pop("intent_groups_json", "[]"), [])
    data["keywords"] = loads(data.pop("keywords_json", "[]"), [])
    data["intent_groups"] = intent_groups or list(data["keywords"])
...
def assigned_article(article: dict[str, Any], assignments: list[dict[str, Any]]) -> bool:
    for assignment in assignments:
        intent_group_ids = assignment.get("intent_group_ids") or []
        intent_groups = assignment.get("intent_groups") or []
        keywords = assignment.get("keywords") or []
        article_types = assignment.get("article_types") or []
        if intent_group_ids:
            group_ok = article.get("intent_group_id") in intent_group_ids
            if not group_ok and intent_groups:
                group_ok = article["intent_group"] in intent_groups
        elif intent_groups:
            group_ok = article["intent_group"] in intent_groups
        else:
            group_ok = not keywords or article["keyword"] in keywords
