...
- **GEO 撰文工作台**：上传项目资料，生成项目信息抽取、内容矩阵、Brief、正文，并完成正文审核与定稿归档。
...
├── mindsun-geo-content-flow/   # 当前默认 GEO 内容生产 Skill 与新版规则库
├── using_skill/                # 新版规则原始素材目录，运行时不直接读取
...
- `PLANNING_*`：项目信息抽取、内容矩阵和外部矩阵 PDF 识别使用。
