...
def add_intent_matrix(repository: ProjectRepository, project_id: str, *intent_groups: str) -> None:
    repository.update_step(
        ...
        output={
            "intent_groups": [
                {"id": keyword, "name": keyword, "keywords": [keyword], "article_types": ["榜单推荐文"]}
...
def create_publishing_db(path: Path, *, with_intent_group: bool = False) -> str:
    database_url = f"sqlite:///{path}"
    engine = create_engine(database_url, future=True)
    with engine.begin() as conn:
        intent_group_column = "intent_group TEXT NOT NULL," if with_intent_group else ""
        conn.execute(
            text(
                f"""
...
def insert_article(
    ...
    *,
    ...
    keyword: str = "关键词 A",
    article_type: str = "榜单推荐文",
    active: bool = True,
):
    columns = {row[1] for row in conn.exec_driver_sql("PRAGMA table_info(article_snapshots)").all()}
    if "intent_group" in columns:
        conn.execute(
            text(
                """
                INSERT INTO article_snapshots (article_id, project_id, intent_group, keyword, article_type, active)
                VALUES (:article_id, :project_id, :intent_group, :keyword, :article_type, :active)
                """
            ),
            {
                "article_id": article_id,
                "project_id": project_id,
                "intent_group": intent_group or keyword,
                "keyword": keyword,
...
def test_usage_summary_groups_by_intent_group_when_column_exists(tmp_path: Path):
    database_url = create_publishing_db(tmp_path / "publishing.db", with_intent_group=True)
    engine = create_engine(database_url, future=True)
    with engine.begin() as conn:
        insert_article(conn, "article-a", intent_group="推荐决策类", keyword="关键词 A")
        insert_article(conn, "article-b", intent_group="推荐决策类", keyword="关键词 B")
    engine.dispose()

    summary = PublishingUsageService(database_url).usage_summary("project-1")

    assert summary["articles"][0]["intent_group"] == "推荐决策类"
    assert summary["articles"][1]["intent_group"] == "推荐决策类"
