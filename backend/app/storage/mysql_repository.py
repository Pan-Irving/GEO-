...
class MySQLProjectRepository(ProjectRepository):
    ...
    def load_project(self, project_id: str) -> Project:
        ...
        data = {
            ...
            "steps": {step: raw_steps.get(step, StepState().model_dump()) for step in STEP_ORDER},
            "jobs": [job for job in jobs if job.get("step") in STEP_ORDER],
        }
            ...
            rows.append(
                {
                    ...
                    "markdown": text(item.get("markdown") or item.get("body")),
                    "raw_json": item,
